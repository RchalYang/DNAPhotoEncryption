...
uint8_t* dna_add(uint8_t* a, uint8_t* b, int size){
    ...
    dna_add_kernel<<<roundup(size,blockSize),blockSize>>>(a,b,size);
    cudaFree(b);
...
__global__ void dna_encode_kernel_lmatrix(float* d_input, uint8_t* d_output, int size){
    ...
    float temp_data = d_input[myIdx];int temp = (int(temp_data*256.0))%4;
    switch (temp){
        case 0:
            d_output[myIdx] = 1;
            break;
...
uint8_t* image_to_dna_matrix_uint8(uint8_t* d_input, float* d_reference, uint8_t* h_rules, int input_size, int cols){
	...
	dna_encode_kernel_image <<< roundup(4*input_size,blockSize), blockSize >>> (d_input, d_reference, d_rules, d_matrix,input_size, cols);
...
uint8_t* image_to_dna_matrix_float(float* d_input, int input_size){
    ...
    dna_encode_kernel_lmatrix <<< roundup(input_size,blockSize), blockSize >>> (d_input,d_matrix,input_size);
...
uint8_t* dna_matrix_to_image(uint8_t* d_input_matrix, int size){
    int blockSize = 64;

    uint8_t* d_image;
    cudaMalloc(&d_image,sizeof(uint8_t)*size);

    dna_matrix_decode_kernel <<< roundup(size,blockSize), blockSize >>> (d_input_matrix,d_image,size);
