...
void rotate(uint8_t* dna_map, int rows, int cols, RC* R, RC* C){
    int blockSize = 64;
    
    uint8_t* d_map;
    uint8_t* d_out;
    cudaMalloc(&d_map,sizeof(uint8_t)*4*cols*rows);
    cudaMalloc(&d_out,sizeof(uint8_t)*4*cols*rows);
    cudaMemcpy(d_map, dna_map, sizeof(uint8_t)*4*cols*rows, cudaMemcpyHostToDevice);
    ...
    rotate_kernel<<<ceil((4.0*cols*rows)/ (1.0* blockSize)),blockSize >>> (d_map,d_out,rows,cols,d_R,d_C);

    cudaMemcpy(dna_map, d_out, sizeof(uint8_t)*4*cols*rows, cudaMemcpyDeviceToHost);
